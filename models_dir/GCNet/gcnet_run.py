...
if __name__ == '__main__':
    ...
    # input data 
    curr_path = os.getcwd()
    src_path = curr_path + "/models_dir/GCNet/source.pcd"
    tgt_path = curr_path + "/models_dir/GCNet/target.pcd"
    source, target = read_cloud(src_path), read_cloud(tgt_path)
    ...
    tg_pcd = o3d.io.read_point_cloud(tgt_path)
    src_pcd = src_pcd.voxel_down_sample(voxel_size=args.voxel_size)
    tg_pcd = tg_pcd.voxel_down_sample(voxel_size=args.voxel_size)
    # registration
    T = model.pipeline(source, target, npts=args.npts)


    end_time = time.time() - time_start
    np.save("gcnet_t.npy",T)
    print(end_time)
